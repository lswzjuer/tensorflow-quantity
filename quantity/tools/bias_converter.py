...
class BiasConverter:
    ...
    def generate_new_bitfile(self, bias_bit, new_bias_bits, is_pruned=False):
        ...
        params_dir=os.path.join(os.path.dirname(self._blob_table),"params.npz")
        npz_file = np.load(params_dir, allow_pickle=True)
        ...
        new_params_dir=os.path.join(os.path.dirname(self._blob_table),"new_params.npz")
        np.savez(new_params_dir, **new_npz_file)
        print("Done!")
