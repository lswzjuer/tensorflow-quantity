...
if __name__=="__main__":
    model_path=r'G:\codeing\Fabu\fuwuqi\tf-reference-model\lib\tensorflow_model\01_semseg\mscoco_fcn_padded_lscape_prediction_tf_graph.pb'
    task_id='seg'
    image_path=r'G:\codeing\Fabu\Bosch_quantize\seg_data\COCO_val2014_000000001993.jpg'

    graph,graph_def=load_graph(model_path)
    print_ops(graph)
    named_feats = get_activations(graph, image_path, task_id)
    print(named_feats)
    print('\n')

    net_info=build_net(graph_def)
    print(net_info)
    print('\n')

    net_pruned=prune_net_info(net_info,named_feats)
    print(net_pruned)
    print('\n')
    bottom_feat_names = get_merge_groups(net_pruned)

    print(bottom_feat_names)
    print('\n')